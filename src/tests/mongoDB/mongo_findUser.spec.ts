...
describe('MongoDB Connection and Operation', () => {
    ...
    it('fund user using mongoDB query', async () => {
        const users = db.collection('users');
        //  console.log(users, "users collection")
        const user = await users.findOne({ name: "Kathy Robel" })
        console.log(user, "found user");
        expect(user.name).toBe("Kathy Robel")
    });
    it('create new user with imported data', async () => {
        ...
        try {
            ...
            let findUser = await users.findOne({_id:new ObjectId(userData._id)})
